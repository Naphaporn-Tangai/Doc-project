...
Template.districtapprovecgex.onCreated(function init() {

    $('body').addClass('waitMe_body');
    var elem = $('<div class="waitMe_container img"><div style="background:url(\'img.svg\')"></div></div>');
    $('body').prepend(elem);
})
Template.districtapprovecgex.onRendered(function () {
    ...
    Session.set('getZonelistApproveCGex', null)
    Session.set("specialisation", null)
    $('#yearselect').val(new Date().getFullYear() + 543)

    // console.log($('#yearselect').val(), Session.get('user').ZONE);
    // Meteor.call('getZonelistApproveCGex', parseInt($('#yearselect').val()) - 543, Session.get('user').ZONE.toString(), function(error, result) {
    //     // console.log(result);
    ...
    Meteor.call('getAllDistrictByAmphoeProvinceReport', Session.get('user').DISTRICT, Session.get('user').PROVINCE, function (error, success) {
        if (error) {
            console.log('error', error);
        }
        if (success) {
...
Template.districtapprovecgex.helpers({
    user() {
        return Session.get('user').NAME
    },
    list() {
        return Session.get('getZonelistApproveCGex')
    },
    count() {
    ...
    listamphoe() {
        return Session.get('user').DISTRICT
    },
    listdistric() {
...
Template.districtapprovecgex.events({
    'change #yearselect'() {
    ...
    "click #find"() {
        ...
        var province = Session.get('user').PROVINCE
        var amphoe = Session.get('user').DISTRICT
        ...
        console.log(amphoe);
        console.log(tambon);
        Meteor.call('getZonelistApproveCGex', year, zone, province, amphoe, tambon, function (error, success) {
            if (error) {
                console.log('error', error);
            }
            if (success) {
                Session.set('getZonelistApproveCGex', success)
                reloadmytable();
            }
        });
    },

...
Template.formregcc.onRendered(function helloOnCreated() {
...
Template.formregcc.onRendered(function helloOnCreated() {
    ...
    Meteor.call('getAllDLA_CODE', function (error, result) {
        ...
        $('#dla_local')[0].selectize.destroy()
        $('#dla_local').selectize({ options: result, create: false });
    });
    Meteor.call('getRegProvince_name', function (error, result) {
        $('#province_name')[0].selectize.destroy()
        $('#province_name').selectize({ options: result, create: false });
...
Template.formregcc.helpers({
    showimg() {
        return Session.get('showImageRegCC');
    },
    showaddress() {
        try {
    ...
    showcaremanager() {
        return Session.get('caremanager')
    }
})

Template.formregcc.events({
    "click .optradio"() {
        
        if($("input[name='optradio']:checked").val() == 0){
            $('#show_radio_company').css('display', 'block')
            $('#show_radio_dla').css('display', 'none')
        }else{
            $('#show_radio_dla').css('display', 'block')
            $('#show_radio_company').css('display', 'none')
        }
    },
    "click .care_optradio"() {
        if($("input[name='care_optradio']:checked").val() == 1){
            Session.set('caremanager', true);
        }else{
            Session.set('caremanager', false);
            Session.set('caremanager_data', 'สาธารณาสุข');
        }
    },
    "click .optradio_ch"() {
        if($("input[name='optradio_ch']:checked").val() == 0){
            Session.set('caremanager_data', 'อบต.');
        }else{
            Session.set('caremanager_data', 'เทศบาล');
        }
    },
    ...
    "change #dla_local"() {
        Session.set('getDLA_CODEbySelected', $('#dla_local').val())
        Meteor.call('getDLA_NAME', $('#dla_local').val(), function (error, result) {
            Session.set('districtName', result)
            Meteor.call('getZoneByDLA', result.PROVINCE, function (error, result) {
                Session.set('getZoneByDLA', result);
            })
    ...
    "change #pid"() {
        ...
        if (checkID($('#pid').val())) {
            Meteor.call('getUserCC', $('#pid').val(), function (error, result) {
    ...
    "click #save"() {
        // var insert_data = {};
        var checkcompany = Session.get('HOSPPCODE') || Session.get('getDLA_CODEbySelected')
        var caremanager = Session.get('caremanager_data')
        if (!$("#pid").val()) {
            ...
            toastr.warning("กรุณากรอกข้อมูล Care Manager", "ไม่สำเร็จ");
        } else {
            ...
            Meteor.call('getDLA_NAME', $('#dla').val(), function (err, res) {
                if (res) {
                    var insert_dla = {
                        ...
                        IMG: Session.get('showImageRegCC'),
                ...
                if (navigator.onLine && !_.isEmpty(insert_dla)) {
                    Meteor.call('INSERT_CC_REGISTER', insert_dla, function (error, result) {
        ...
        UploadFS.selectFiles(function (file) {
            ...
            let uploader = new UploadFS.Uploader({
                ...
                onComplete(file) {
                    ...
                    Session.set("showImageRegCC", file.url)
                },
                onCreate(file) {
                ...
                onStart(file) {
                    ...
                    Session.set("showImageRegCC", "InternetSlowdown_Day.gif")
                },
                onStop(file) {

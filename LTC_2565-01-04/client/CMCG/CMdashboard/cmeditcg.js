...
Template.cmeditcg.onCreated(function init() {
...
Template.cmeditcg.onRendered(function () {
    ...
    Meteor.call('getUserCG', Session.get('CMEditedDetailCG').CID, function (error, result) {
        Session.set('getEditProfileCG', result)
        Session.set("showImgCMEditedCG", result[0].IMG)
...
Template.cmeditcg.helpers({
    ...
    profile() {
        if (Session.get('CMEditedDetailCG')) {
            return Session.get('CMEditedDetailCG')
        }
    },

    showimg() {
        if (Session.get('showImgCMEditedCG'))
            return Session.get('showImgCMEditedCG');
    },
    EXPIREDATE() {
...
Template.cmeditcg.events({
    'click #back'() {
        ...
        setTimeout(() => {
            Session.set("showImgCMEditedCG", null)
        }, 500);
    },
    "click  #radio_company"() {
        Session.set('switchCompany', true)
    ...
    "click  #radio_dla"() {
        Session.set('switchCompany', false)
        Session.set('hospCodeEditedCGByCM', "");
    },
    ...
    "change #companyName"() {
        Meteor.call('getAllServiceCenterDistrict', $('#companyName').val(), function (error, result) {
            if (result) {
                Session.set('hospCodeEditedCGByCM', result)
                Session.set('hospAddCGEdited', result.address + " หมู่ " + result.moo + " ต." + result.district.split('-')[1] + " อ." + result.amphoe.split('-')[1] + " จ." + result.province.split('-')[1] + " เขต " + result.zone)
            }
        });
    },
    ...
    "click #save"() {
        ...
        if (s_center && $("#subdistrict_name").val() && $("#TRAININGHOURS").val() && $('#province_name') && $('#district_name') && $('#subdistrict_name') && $("#pid").val() && $("#titleName").val() && $("#FName").val() && $("#LName").val() && $("#sex").val() && $("#tel").val() && $("#email").val() && $("#trainingBy").val() && $("#birthDate").val() && $("#trainingDate").val()) {
            ...
            var data = Session.get('hospCodeEditedCGByCM');

            if (data) {
                Meteor.call('getAllServiceCenterDistrict', $('#companyName').val(), function (err, res) {
                    CG_REGISTER.update({
                        "_id": Session.get('CMEditedDetailCG')._id
                    }, {
                            $set: {
    ...
    'click button[name=upload]'(ev) {
        console.log(Session.get("removeImgCMEditedCG"))
        Photos.remove({ _id: Session.get("removeImgCMEditedCG") })
        UploadFS.selectFiles(function (file) {
            ...
            let uploader = new UploadFS.Uploader({
                ...
                onComplete(file) {
                    ...
                    Session.set("showImgCMEditedCG", file.url)
                },
                onCreate(file) {
                    // console.log(file.name + ' has been created with ID ' + file._id);
                    Session.set("removeImgCMEditedCG", file._id);
                },
                ...
                onStart(file) {
                    ...
                    Session.set("showImgCMEditedCG", "InternetSlowdown_Day.gif")
                },
                onStop(file) {

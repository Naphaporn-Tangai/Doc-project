...
Template.betterElder.onCreated(function helloOnCreated() {
    $('body').addClass('waitMe_body');
    var elem = $('<div class="waitMe_container img"><div style="background:url(\'img.svg\')"></div></div>');
    $('body').prepend(elem);
});
Template.betterElder.onRendered(function helloOnCreated() {
    $('#kkk').show();
    datatables(window, $);
    ...
    Meteor.call('LISTBETTERELDER', service_center, "02", function (error, result1) {
        Session.set('LISTBETTERELDER', result1);
        setTimeout(function () {
...
Template.betterElder.helpers({
    ...
    listelder() {
        return Session.get('LISTBETTERELDER');
    },
    nameCen() {
    ...
    coutElder() {
        return Session.get('LISTBETTERELDER') ? Session.get('LISTBETTERELDER').length : 0;
    }
});

Template.betterElder.events({
    ...
    'click #remove'() {
        if (confirm('ต้องการลบใช่หรือใหม่')) {
            ...
            if (Session.get('getProfileCM')[0].HOSPCODE) {
                Meteor.call('LISTBETTERELDER', Session.get('getProfileCM')[0].HOSPCODE.CODE, function (error, result) {
                    Session.set('LISTBETTERELDER', result);
                })
            } else {
                Meteor.call('LISTBETTERELDER', Session.get('getProfileCM')[0].DLACODE.CODE, function (error, result) {
                    Session.set('LISTBETTERELDER', result);
                })
            }
        }

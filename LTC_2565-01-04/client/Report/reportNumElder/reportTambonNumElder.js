...
Template.reporttambonnumelder.onRendered(function () {
    ...
    //*************************************************************
    console.log(Session.get('ZoneFindTambonElder'))
    ...
    Meteor.call('countElderByTambon',Session.get('ZoneFindTambonElder'),isNhso, function (error, result) {
      
        var newData = []
        
        _.each(result, function (x) {
            newData.push({ zone: x._id.tambon, group: x._id.group, count: x.count })
        })
        ...
        Session.set('graphDatatambonNumElder', data)
        //console.log(data);
        ...
        Session.set('finalTambonNumElder',newgroup)
    });


});

Template.reporttambonnumelder.helpers({
    zonerep1() {
       
        return Session.get('finalTambonNumElder').sort();
    },
    isNhso(){
        var isNhso = Session.get('IsNHSOElder') ? false : true
        return isNhso
    ...
    sumCMCG() {
        var sum = Session.get('finalTambonNumElder')
    ...
    zone() {
        ...
        return Session.get('ZoneFindTambonElder')
    }
});

Template.reporttambonnumelder.events({
    'click #printrep'() {
        Session.set('sendToNumTambonElderPrint', { table: Session.get('finalTambonNumElder'), graph: Session.get('graphDatatambonNumElder') })
        Router.go('/printtambonelder')
    },
    'click #viewprovince'() {
        var data = this.z
        Session.set('ZoneFindHospElder', data);

...
Template.printtambonelder.onRendered(function () {

    var start = +new Date();
    
    var data = Session.get('sendToNumTambonElderPrint').graph
...
Template.printtambonelder.helpers({
    zonerep1() {
        return _.sortBy(Session.get('sendToNumTambonElderPrint').table, o => o._id)
    },
    sumCMCG() {
        var sum = Session.get('sendToNumTambonElderPrint').table
...
Router.route('/printtambonelder', function () {
    this.render('printtambonelder');
});
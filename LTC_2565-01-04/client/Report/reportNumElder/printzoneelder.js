...
Template.printzoneelder.onRendered(function () {

    var start = +new Date();

    var data = Session.get('sendToNumZoneElderPrint').graph
    var ctx = document.getElementById("zonecm").getContext("2d");
    // f_cmamphoe = f_cmamphoe.sort()
    console.log(Session.get('sendToNumZoneElderPrint'))
    var cdata = {
        labels: data.labels,
...
Template.printzoneelder.helpers({
    zonerep1() {
        return _.sortBy(Session.get('sendToNumZoneElderPrint').table, o => o._id)
    },
    sumCMCG() {
        var sum = Session.get('sendToNumZoneElderPrint').table
...
Router.route('/printzoneelder', function () {
    this.render('printzoneelder');
});
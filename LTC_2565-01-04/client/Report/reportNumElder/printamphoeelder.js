...
Template.printamphoeelder.onRendered(function () {

    var start = +new Date();
    
    var data = Session.get('sendToNumAmphoeElderPrint').graph
...
Template.printamphoeelder.helpers({
    zonerep1() {
        return _.sortBy(Session.get('sendToNumAmphoeElderPrint').table, o => o._id)
    },
    sumCMCG() {
        var sum = Session.get('sendToNumAmphoeElderPrint').table
...
Router.route('/printamphoeelder', function () {
    this.render('printamphoeelder');
});
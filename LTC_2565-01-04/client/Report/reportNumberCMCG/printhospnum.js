...
Template.printhospnum.onCreated(function () {
    ...
    var zone = []
    _.each(Session.get('sendToNumHospReport'), function (x) {

        zone.push(x.hospname);
...
Template.printhospnum.helpers({
    zonerep1() {
        return Session.get('sendToNumHospReport')
    },
    zone() {
        if (Session.get('ZoneFindHospCMCG')) {
    ...
    sumCMCG() {
        var sum = Session.get('sendToNumHospReport')

...
Template.printtambonnum.onCreated(function () {
    ...
    var zone = []
    _.each(Session.get('sendToNumTambonReport'), function (x) {
...
Template.printtambonnum.helpers({
    zonerep1() {
        return Session.get('sendToNumTambonReport')
    },
    zone() {
        if (Session.get('ZoneFindTambonCMCG')) {
    ...
    sumCMCG() {
        var sum = Session.get('sendToNumTambonReport')

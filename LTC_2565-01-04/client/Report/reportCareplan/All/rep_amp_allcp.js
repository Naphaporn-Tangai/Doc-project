...
Template.rep_amp_allcp.onRendered(function () {
    ...
    var zone = Session.get('ViewZoneAllCp'), province = Session.get('ViewProvinceAllCp'), amp = null, tam = null
    Meteor.call('countAllReportCP', year, zone, province, amp, tam, function (error, result2) {
        Session.set('countAllCpAmp', result2)
        Meteor.call('countAllElderCP', year, zone, province, amp, tam, function (error, result) {

            var concatArr = _.concat(Session.get('countAllCpAmp'), result)
            ...
            Session.set('AmpElderAllCp', output)
...
Template.rep_amp_allcp.helpers({
    zonerep1() {
        return _.sortBy(Session.get('AmpElderAllCp'), o => o._id)

    },
    sumCMCG() {
        var data = Session.get('AmpElderAllCp')

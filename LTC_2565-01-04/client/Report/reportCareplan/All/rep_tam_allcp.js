...
Template.rep_tam_allcp.onRendered(function () {
   ...
   var zone = Session.get('ViewZoneAllCp'), province = Session.get('ViewProvinceAllCp'), amp = Session.get('ViewAmphoeAllCp'), tam = null
    Meteor.call('countAllReportCP', year, zone, province, amp, tam, function (error, result2) {
        Session.set('countAllCpTam', result2)
        Meteor.call('countAllElderCP', year, zone, province, amp, tam, function (error, result) {

            var concatArr = _.concat(Session.get('countAllCpTam'), result)
            ...
            Session.set('TamElderAllCp', output)
...
Template.rep_tam_allcp.helpers({
    zonerep1() {
        return _.sortBy(Session.get('TamElderAllCp'), o => o._id)

    },
    sumCMCG() {
        var data = Session.get('TamElderAllCp')
...
Template.rep_tam_allcp.events({
    ...
    'click #viewprovince'() {
        Session.set('ViewHospAllCp', this._id)
        Router.go('/rep_hosp_allcp')
    }
});

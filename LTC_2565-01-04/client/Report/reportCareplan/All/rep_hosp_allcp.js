...
Template.rep_hosp_allcp.onCreated(function () {
    $('body').addClass('waitMe_body');
    var elem = $('<div class="waitMe_container img"><div style="background:url(\'img.svg\')"></div></div>');
    $('body').prepend(elem);
});
Template.rep_hosp_allcp.onRendered(function () {
    ...
    var zone = Session.get('ViewZoneAllCp'), province = Session.get('ViewProvinceAllCp'), amp = Session.get('ViewAmphoeAllCp'), tam = Session.get('ViewHospAllCp')
    ...
    Meteor.call('countAllReportCP', year, zone, province, amp, tam, function (error, result2) {
        ...
        Meteor.call('countAllElderCP', year, zone, province, amp, tam, function (error, result) {
            ...
            Meteor.call('Replace_ServiceCenterCode', output, function (error, result2) {
                ...
                Session.set('HospElderAllCp', _.sortBy(result2, o => o._id))
...
Template.rep_hosp_allcp.helpers({
    zonerep1() {
        return _.sortBy(Session.get('HospElderAllCp'), o => o._id)

    },
    sumCMCG() {
        var data = Session.get('HospElderAllCp')
...
Template.rep_hosp_allcp.events({
    'click #printrep'() {
        ...
        Session.set('isHospPrintAllRepCp', true);
        Session.set('sendToPrintAllRepCp', Session.get('HospElderAllCp'))
        ...
        Meteor.call('countAllReportCP', events.target.value, zone, province, amp, tam, function (error, result2) {
            ...
            Meteor.call('countAllElderCP', events.target.value, zone, province, amp, tam, function (error, result) {
                ...
                Meteor.call('Replace_ServiceCenterCode', output, function (error, result2) {
                    ...
                    Session.set('HospElderAllCp', result2)
                    $('body.waitMe_body').addClass('hideMe');
                    $('body.waitMe_body').find('.waitMe_container:not([data-waitme_id])').remove();
                    $('body.waitMe_body').removeClass('waitMe_body hideMe');
                })
    ...
    // 'click #viewprovince'() {
    //     Session.set('ViewAmphoeAllCp', this._id)

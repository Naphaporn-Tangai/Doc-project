...
Template.reportzonecp.onRendered(function () {
    ...
    Meteor.call('countCPbyZone', year, function (error, result2) {
        Session.set('countCPbyZone', result2)
        Meteor.call('countElderCPbyZone', year, function (error, result) {

            var concatArr = _.concat(Session.get('countCPbyZone'), result)
            ...
            Session.set('FinalZoneElderCp', output)
...
Template.reportzonecp.events({
    'click #printrep'() {
        Session.set('sendToPrintReport2', Session.get('FinalZoneElderCp'))
        Router.go('/printzonecp')
    },
    'change #budgetyear'(events) {
        ...
        Meteor.call('countCPbyZone', events.target.value, function (error, result2) {
            Session.set('countCPbyZone', result2)
    
            Meteor.call('countElderCPbyZone',  events.target.value, function (error, result) {
    ...
    'click #viewprovince'(){
        Session.set('ZoneViewProvinceCP', this._id)
        Router.go('/reportprovincecp')
    }
});

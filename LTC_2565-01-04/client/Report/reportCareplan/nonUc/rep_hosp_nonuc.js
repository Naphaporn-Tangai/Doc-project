...
Template.rep_hosp_nonuc.onCreated(function () {
    $('body').addClass('waitMe_body');
    var elem = $('<div class="waitMe_container img"><div style="background:url(\'img.svg\')"></div></div>');
    $('body').prepend(elem);
});
Template.rep_hosp_nonuc.onRendered(function () {
    ...
    var zone = Session.get('ViewProvinceCPNonUc'), province = Session.get('ViewAmphoeCPNonUc'), amp = Session.get('ViewTambonCPNonUc'), tam = Session.get('ViewHospCPNonUc')

    Meteor.call('countCPNonUc', year, zone, province, amp, tam, function (error, result2) {
        Session.set('countCPNonUcHosp', result2)

        Meteor.call('countElderNonUc', year, zone, province, amp, tam, function (error, result) {
            var concatArr = _.concat(Session.get('countCPNonUcHosp'), result)
            ...
            Meteor.call('Replace_ServiceCenterCode', output, function (error, result2) {
                ...
                Session.set('HospElderCpNonUc', _.sortBy(result2, o => o._id))
...
Template.rep_hosp_nonuc.helpers({
    zonerep1() {
        return _.sortBy(Session.get('HospElderCpNonUc'), o => o._id)

    },
    sumCMCG() {
        var data = Session.get('HospElderCpNonUc')
...
Template.rep_hosp_nonuc.events({
    ...
    'click #printrep'() {
        ...
        Session.set('isHospPrintCpNonUC',true);
        Session.set('sendToPrintCpNonUC', Session.get('HospElderCpNonUc'))

...
Template.rep_amp_nonuc.onRendered(function () {
    ...
    var zone = Session.get('ViewProvinceCPNonUc'), province = Session.get('ViewAmphoeCPNonUc') , amp = null, tam = null
    Meteor.call('countCPNonUc', year, zone, province, amp, tam, function (error, result2) {
        Session.set('countCPNonUcAmp', result2)
        Meteor.call('countElderNonUc', year, zone, province, amp, tam, function (error, result) {

            var concatArr = _.concat(Session.get('countCPNonUcAmp'), result)
            ...
            Session.set('AmpElderCpNonUc', output)
...
Template.rep_amp_nonuc.helpers({
    zonerep1() {
        return _.sortBy(Session.get('AmpElderCpNonUc'), o => o._id)

    },
    sumCMCG() {
        var data = Session.get('AmpElderCpNonUc')
...
Template.rep_amp_nonuc.events({
    ...
    'click #viewprovince'() {
        Session.set('ViewTambonCPNonUc', this._id)
        Router.go('/rep_tam_nonuc')
    }
});

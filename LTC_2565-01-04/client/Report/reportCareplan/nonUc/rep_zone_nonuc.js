...
Template.repzone_non_uc.onCreated(function () {
    $('body').addClass('waitMe_body');
    var elem = $('<div class="waitMe_container img"><div style="background:url(\'img.svg\')"></div></div>');
    $('body').prepend(elem);
});
Template.repzone_non_uc.onRendered(function () {
    $('.dropdown-submenu a.test').on("click", function (e) {
        $(this).next('ul').toggle();
        e.stopPropagation();
        e.preventDefault();
    });
    Session.set('ViewAmphoeCPNonUc', null)
    Session.set('ViewProvinceCPNonUc', null)
    Session.set('BudgetYearNonUcCp',null)
    ...
    Session.set('isTamPrintCpNonUC',false);
    ...
    Meteor.call('countCPNonUc', year, zone, province, amp, tam, function (error, result2) {
        Session.set('countCPNonUc', result2)
        Meteor.call('countElderNonUc', year, zone, province, amp, tam, function (error, result) {

            var concatArr = _.concat(Session.get('countCPNonUc'), result)
            ...
            Session.set('FinalElderCpNonUc', output)
...
Template.repzone_non_uc.helpers({
    zonerep1() {
        return _.sortBy(Session.get('FinalElderCpNonUc'), o => o._id)

    },
    sumCMCG() {
        var data = Session.get('FinalElderCpNonUc')
...
Template.repzone_non_uc.events({
    'click #printrep'() {
        Session.set('isZonePrintCpNonUC',true);
        Session.set('isProPrintCpNonUC',false);
        Session.set('isAmpPrintCpNonUC',false);
        Session.set('isTamPrintCpNonUC',false);
        Session.set('isHospPrintCpNonUC',false);
        Session.set('sendToPrintCpNonUC', Session.get('FinalElderCpNonUc'))
    ...
    'change #budgetyear'(events) {
        ...
        $('#graphcp').remove();
        $('#graph-cp').append('<canvas id="graphcp" style="width:100%;height:320px"></canvas>');
        Session.set('BudgetYearNonUcCp',events.target.value)
        Meteor.call('countCPNonUc', events.target.value, function (error, result2) {
            Session.set('countCPNonUc', result2)

            Meteor.call('countElderNonUc', events.target.value, function (error, result) {
    ...
    'click #viewprovince'() {
        Session.set('ViewProvinceCPNonUc', this._id)
        Router.go('/repzone_non_uc')
    }
});

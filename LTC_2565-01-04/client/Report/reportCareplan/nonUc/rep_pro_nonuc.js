...
Template.rep_pro_nonuc.onRendered(function () {
   ...
   //
    ...
    var zone = Session.get('ViewProvinceCPNonUc'), province = null, amp = null, tam = null
    Meteor.call('countCPNonUc', year, zone, province, amp, tam, function (error, result2) {
        Session.set('countCPNonUcPro', result2)
        Meteor.call('countElderNonUc', year, zone, province, amp, tam, function (error, result) {

            var concatArr = _.concat(Session.get('countCPNonUcPro'), result)
            ...
            Session.set('ProElderCpNonUc', output)
...
Template.rep_pro_nonuc.helpers({
    zonerep1() {
        return _.sortBy(Session.get('ProElderCpNonUc'), o => o._id)

    },
    sumCMCG() {
        var data = Session.get('ProElderCpNonUc')
...
Template.rep_pro_nonuc.events({
    ...
    'click #viewprovince'() {
        Session.set('ViewAmphoeCPNonUc', this._id)
        Router.go('/rep_amp_nonuc')
    }
});

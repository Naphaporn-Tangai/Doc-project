...
Template.provinceapprovecgex.onCreated(function init() {

    $('body').addClass('waitMe_body');
    var elem = $('<div class="waitMe_container img"><div style="background:url(\'img.svg\')"></div></div>');
    $('body').prepend(elem);
})
Template.provinceapprovecgex.onRendered(function () {
    ...
    Meteor.call('getAllAmphoeByProvinceReport', Session.get('user').PROVINCENAME, function (error, success) {
        if (error) {
...
Template.provinceapprovecgex.helpers({
    user() {
...
Template.provinceapprovecgex.events({
    'change #yearselect'() {
    ...
    "click #find"() {
        ...
        // console.log(amphoe);
        // console.log(tambon);
        Meteor.call('getZonelistApproveCGex', year, checkZone(zone), province, amphoe, tambon, function (error, success) {
            if (error) {
                console.log('error', error);
            }
            if (success) {
    ...
    setTimeout(function () {
        $('#mytable').DataTable({
            ...
            buttons: [{
                ...
                extend: 'excel',
                text: " ดาวน์โหลด",
                title: "รายชื่่อ CM รอนุมัติเขต" + checkZone(Session.get('user').ZONE) + "_" + moment().format('DD-MM-YY') + "",
                exportOptions: {
